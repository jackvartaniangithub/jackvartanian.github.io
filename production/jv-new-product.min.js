    ...
    try {
        ...
        let NewProduct = {
            init: () => {
                ...
                NewProduct.buildCollectionInfo();
            ...
            buildCollectionInfo: () => {
                ...
                if (vtxctx.departmentName == 'Adega') {
                    console.info("Categoria de Adega indentificada. Encerrando [buildCollectionInfo]");
                    return;
                }

                if (!$(".x-specification .value-field.Colecao").length) {
                    console.info("[Product] - Nome da coleção não localizado. Encerrando [buildCollectionInfo]");
                    return;
                }

                let thisColectionName = $(".x-specification .value-field.Colecao").text();

                $.ajax({
                    type: "GET",
                    url: "/arquivos/000-JML-jsonColecoes.js",
                    dataType: "json"
                }).done(function (colecaoJson) {
                    for (let col in colecaoJson) {
                        if (colecaoJson[col] == thisColectionName) {
                            console.log("Achei!");
                            let textoColecao = colecaoJson[nomeColecao]["desc"];
                            let bannerColecao = colecaoJson[nomeColecao]["bannerTop"];
                            let linkColecao = colecaoJson[nomeColecao]["link"];
                            ...
                            colecaoComLink = '<h3>Coleção</h3><a style="font-size: 14px; cursor:pointer; font-weight:400; color: #393939;" href="' + linkColecao + '">' + nomeColecao + '</a>'
                            $('.conteudo .colecao').append(colecaoComLink);
                        }
                    }
                });
            ...
            skuDescricoes: () => {
                var descriptions = [
                    {
                        "PU02007T": "A Pulseira Chain Rock Ã© perfeita para dar um up em qualquer look! \
                         Do dia para a noite, com ela nÃ£o tem erro!\n \
                        Tamanho P contÃ©m 14 elos, contando com fecho."
                    },
                    {
                        "PU02008T": "A Pulseira Chain Rock Ã© perfeita para dar um up em qualquer look! \
                         Do dia para a noite, com ela nÃ£o tem erro!\n \
                        Tamanho M contÃ©m 15 elos, contando com fecho."
                    },
                    {
                        "PU02009T": "A Pulseira Chain Rock Ã© perfeita para dar um up em qualquer look! \
                         Do dia para a noite, com ela nÃ£o tem erro!\n \
                        Tamanho G contÃ©m 16 elos, contando com fecho."
                    },
                    {
                        "PU01791T-": "A Pulseira Chain Elo P - Tamanho P possui um acabamento impecÃ¡vel e ultra \
                         leveza, capaz de dar forÃ§a a qualquer look sem perder o ar luxuoso\n \
                         Tamanho P contÃ©m 14 elos, contando com fecho."
                    },
                    {
                        "PU01801T-": "A Pulseira Chain P - Tamanho M possui um acabamento impecÃ¡vel e ultra \
                        leveza, capaz de dar forÃ§a a qualquer look sem perder o ar luxuoso\n \
                        Tamanho M contÃ©m 15 elos, contando com fecho."
                    },
                    {
                        "PU01803T-": "A Pulseira Chain P - Tamanho G possui um acabamento impecÃ¡vel e ultra \
                        leveza, capaz de dar forÃ§a a qualquer look sem perder o ar luxuoso\n \
                        Tamanho G contÃ©m 16 elos, contando com fecho."
                    }
                ]
            
                var skuConsulta = $('.skuReference')[0].innerText;
            
                function buscaDescricaoSku(skuReference) {
                    for (var i = 0; i < descriptions.length; i++) {
                        for (var key in descriptions[i]) {
                            if (descriptions[i].hasOwnProperty(key)) {
                                var description = descriptions[i][key];
                                if (key === skuReference) {
                                    return description;
                                }
                            }
                ...
                var descPersonalizada = buscaDescricaoSku(skuConsulta);
            
                function changeDescription(descPersonalizada) {
                    $('.productDescription')[0].innerText = "";
                    $('.productDescription')[0].innerText = descPersonalizada;
                }
            
                typeof (descPersonalizada) === "string" ? changeDescription(descPersonalizada) : null;
            }
        };
